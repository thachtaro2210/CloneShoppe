...
export default function AsideFilter({ setFilters, categories }: AsideFilterProps) {
  const { t } = useTranslation('asideFilter'); // Sử dụng namespace 'asideFilter'
  ...
  return (
    <div className="py-4 px-2">
      ...
      <div className="text-sm font-medium mb-2">{t('priceRange')}</div>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        <div className="flex items-center gap-2">
         <Input
            type="text"
            placeholder={t('fromPrice')} // Sử dụng t() cho placeholder
            name="fromPrice"
            register={register}
            className="flex-1"
          ...
          <Input
            type="text"
            placeholder={t('toPrice')} // Sử dụng t() cho placeholder
            name="toPrice"
            register={register}
            className="flex-1"
            classNameInput="p-1 w-full outline-none border border-gray-300 focus:border-gray-500 rounded-sm focus:shadow-sm text-sm"
      ...
      <ul className="space-y-2 mb-4">
        {[5, 4, 3, 2, 1].map((starCount) => (
          <li key={starCount}>
            <button className="flex items-center text-sm" onClick={() => handleRatingFilter(starCount)}>
              {Array(starCount)
                .fill(0)
                .map((_, index) => (
                  <svg
